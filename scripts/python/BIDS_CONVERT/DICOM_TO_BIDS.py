...
def main(args,run_dict,searchpath):
    ...
    # Get a nifti conversion of the data and save a temp file to be put into bids folder later
    #nifti_img = nib.Nifti1Image(dicom_data.pixel_array, affine=None)
    #nib.save(nifti_img, args.tmpfile)
    ...
    #group_number   = 0x0008
    #element_number = 0x0022
    #tag            = pydicom.tag.Tag(group_number, element_number)
    #acq_date       = int(dicom_data[tag].value)
    acq_date = metadata["AcquisitionTime"]
    
    # Get the series value
    #group_number   = 0x0008
    #element_number = 0x103e
    #tag            = pydicom.tag.Tag(group_number, element_number)
    #series         = dicom_data[tag].value.lower()
    ...
    for idx,ifile in enumerate(ifile_paths):
        ...
        """
        # Handle side car exceptions
        if modality == 'dwi':
            entities['extension'] = '.bval'
            Bval_path             = args.bidsroot+build_path(entities=entities, path_patterns=patterns)
            np.savetxt(Bval_path, Bval, fmt="%0.0f")
    
    
            entities['extension'] = '.bvec'
            Bvec_path             = args.bidsroot+build_path(entities=entities, path_patterns=patterns)
            np.savetxt(Bvec_path, Bvec, fmt="%0.6f")
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bidsroot', help='BIDS Root Directory')
    parser.add_argument('--dataset', help='dataset name')
    parser.add_argument('--datalake', help='Path to the bids datalake for image naming.')
    parser.add_argument('--datefile', help='Path to a file containing acquisition dates for this patient.')
    parser.add_argument('--subject', help='Subject ID.')
    parser.add_argument('--wrapper', default=False, action='store_true', help="Process multiple files at a time.")
